...
export default function HoraStatusEditor({ id, estadoActual }: Props) {
  ...
  const ESTADOS: { value: HoraEstado; label: string; color: string }[] = [
    { value: "borrador", label: isEn ? "Draft" : "Borrador", color: "bg-slate-100 text-slate-600 hover:bg-slate-200" },
    { value: "confirmado", label: isEn ? "Confirmed" : "Confirmado", color: "bg-green-100 text-green-700 hover:bg-green-200" },
    { value: "facturado", label: isEn ? "Billed" : "Facturado", color: "bg-blue-100 text-blue-700 hover:bg-blue-200" },
    { value: "rechazado", label: isEn ? "Rejected" : "Rechazado", color: "bg-red-100 text-red-700 hover:bg-red-200" },
  ];
  ...
  return (
    <div>
      <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">
        {isEn ? "Change status" : "Cambiar estado"}
      </p>
      <div className="flex flex-wrap gap-2">
        {ESTADOS.map((e) => (
          ...
          >
            ...
            {e.label}
            {e.value === estadoActual && <span className="text-[10px] opacity-60">· {isEn ? "current" : "actual"}</span>}
          </button>
        ))}
      </div>
