...
import { deleteUsuario } from "@/lib/sheets/mutations";
...
export async function deleteUserAction(email: string): Promise<{ success: boolean; error?: string }> {
  try {
    await requireAdmin();
    const ctx = await getSheetCtx();
    await deleteUsuario(ctx, email);
    revalidatePath("/admin/usuarios");
    return { success: true };
  } catch (e: unknown) {
