...
import { getRegistrosHoras, getRegistroById } from "@/lib/sheets/queries";
import { updateRegistroEstado, updateRegistroEstadoBulk } from "@/lib/sheets/mutations";
...
import { updateHourRecord, deleteHourRecord } from "@/lib/hours/service";
...
export async function deleteHourAction(id: string): Promise<ActionResult> {
  try {
    ...
    const ctx = await getSheetCtx();
    await deleteHourRecord(ctx, id, usuarioId);
