...
#### Corregido
- `vercel.json`: removido el campo `env` que era un array inválido (debe ser objeto).
- Middleware: `privacy` y `terms` bloqueadas por defecto — ahora accesibles sin login.
- `getClientes` y `getTareas`: parseo robusto del campo booleano `activo` desde Sheets (`trim().toLowerCase()`).
- `calculateHoursAmount`: umbral mensual global (reset el 1° de cada mes, no acumulado histórico).
